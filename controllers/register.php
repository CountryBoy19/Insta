...
/* Default variables */
$register_username = $register_name = $register_email = $register_password = '';
...
if(!empty($_POST)) {
    ...
    if(empty($_SESSION['error'])) {
        ...
        $stmt = $database->prepare("INSERT INTO `users` (`username`, `password`, `email`, `email_activation_code`, `name`, `active`, `date`, `api_key`, `points`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('sssssssss', $_POST['username'], $password, $_POST['email'], $email_code, $_POST['name'], $active, $date, $api_key, $settings->store_user_default_points);
        $stmt->execute();
        ...
        if($active == '1') {
            ...
            $email_template = generate_email_template(
                ...
                [
                    '{{ACTIVATION_LINK}}' => $settings->url . 'activate/' . $_POST['email'] . '/' . $email_code,
                    '{{NAME}}' => $_POST['name'],
